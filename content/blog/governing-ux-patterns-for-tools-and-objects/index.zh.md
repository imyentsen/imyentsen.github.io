...
![物件與工具，patterns 必須治理的兩個對象](./objectives.jpg "物件與工具，我們的 patterns 必須治理的兩個對象。")
...
![我在物件工作與後續 pattern 稽核工作中的角色](./role.jpg "我在物件工作，以及後續 pattern 稽核工作中的角色。")
...
![同一種物件類型的數種 List 呈現方式並列](./challenge-objects.jpg "同一種物件類型的數種 List 呈現方式並列。缺乏清晰邏輯的多樣性，傷害了我們的 UX。")
...
![同一個 toolbar 在兩個產品區域中，區塊順序不同](./challenge-tools.jpg "同一個 toolbar 在兩個產品區域中，區塊順序卻不相同。")
...
![協作建立的物件地圖](./objectmap.jpg "協作建立的物件與工具地圖。")
...
![依 components 在產品中的覆蓋範圍排定優先順序](./priorities.jpg "依 components 在產品中的覆蓋範圍排定優先順序。")
...
![試驗性的 Data Label 與 List component](./pilot.jpg "試驗性的 Data Label 與 List component。")
...
![pattern 幫助設計師理解如何使用多種相關 component](./toolbar-pattern.jpg "pattern 來幫助設計師理解哪一類 component 屬於哪一個產品區域、以什麼順序排列，以及允許怎麼樣的互動發生。")
...
![guideline 擷取與稽核流程](./audit-skill.jpg "guideline 擷取與稽核流程。")

第二個 是 audit skill。它以 Scraper Skill 對使用者指定的設計進行審查。可以在 Figma 或是在 Codex 中執行。

![稽核輸出，以 annotation 形式放在 Figma 上](./audit-output.jpg "稽核輸出，以 annotation 形式放在 Figma 上。")

# 成果

## 數位孿生的物件呈現變得清晰易解

![Data Label 在 2D 與 3D 環境中的樣貌](./outcome-objects.jpg "Data Label 在 2D 與 3D 環境中的樣貌。")
...
![Data Label 在 2D 與 3D 環境中的樣貌](./outcome-objects.jpg "Toolbar 改善後的樣貌")

我們將工具分類。並依照互動類型、優先順序建立基本的互動原則。我們也明確指出過往產品中容易出現的 ux pattern 並將其列入文件中。

## 導入 AI 加速 design system audit

![Data Label 在 2D 與 3D 環境中的樣貌](./outcome-objects.jpg "Toolbar annotation")
...
![component 路線圖，Tree、Card 與 Tag](./outcome-roadmap.jpg "component 路線圖。Tree、Card 與 Tag。")
